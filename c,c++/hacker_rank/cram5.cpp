...
void nextMove( char player, int posx, int posy, char board[9][11]) {
	...
	//3 free vertical and in next column either of corner one is free
	for(int i=0;i<7;++i){
		for(int j=0;j<10;++j){
			if(board[i][j]=='-'){
	...
	//3 free vertical and in next column middle one is free
	for(int i=0;i<7;++i){
		for(int j=0;j<10;++j){
			if(board[i][j]=='-'){
				if(board[i+1][j]=='-' && board[i+2][j]=='-' && board[i+1][j+1]=='-'){
	...
	//3 free horizontal and in next line 2 shifted right are free
/*	for(int i=0;i<10;++i){
		for(int j=0;j<7;++j){
			if(board[i][j]=='-'){
				if(board[i][j+1]=='-' && board[i][j+2]=='-' && board[i+1][j+1]=='-'){
